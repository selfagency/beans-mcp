...
describe("handleQueryOperation", () => {
  ...
  it("should handle llm_context with writeInstructions", async () => {
    ...
    expect(result.structuredContent.instructionsPath).toBe(
  ...
  it("should handle filter operation with statuses", async () => {
    vi.mocked(mockBackend.list).mockImplementationOnce(async () =>
      mockBeans.filter((b) => b.status === "in-progress")
    );
    const result = await handleQueryOperation(mockBackend, {
      operation: "filter",
  ...
  it("should handle filter operation with types", async () => {
    vi.mocked(mockBackend.list).mockImplementationOnce(async () =>
      mockBeans.filter((b) => b.type === "feature")
    );
    const result = await handleQueryOperation(mockBackend, {
      operation: "filter",
      types: ["feature"],
  ...
  it("should filter out closed beans in search when includeClosed is false", async () => {
    ...
    expect(result.structuredContent.beans).toHaveLength(2);
    expect(
      result.structuredContent.beans.every(
        (b: BeanRecord) => b.status !== "completed" && b.status !== "scrapped"
      )
    ).toBe(true);
  });
  ...
  it("should handle null tags gracefully", async () => {
    ...
    vi.mocked(mockBackend.list).mockImplementationOnce(
      async () => beansWithoutTags
    );
    const result = await handleQueryOperation(mockBackend, {
      operation: "filter",
      tags: ["some-tag"],
  ...
  it("should handle search without includeClosed filter", async () => {
    ...
    expect(result.structuredContent.beans.length).toBeGreaterThan(0);
  });
});
