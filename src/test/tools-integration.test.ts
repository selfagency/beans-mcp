...
describe('Tool Handler Integration', () => {
  ...
  describe('query handler operations', () => {
    ...
    it('should filter beans by status', async () => {
      const mockBackend = createMockBackend();
      mockBackend.list = vi.fn(async () => mockBeans.filter(b => b.status === 'todo'));
  ...
  describe('error handling', () => {
    ...
    it('should handle backend errors in create', async () => {
      ...
      await expect(mockBackend.create({ title: 'Test', type: 'task' })).rejects.toThrow('Create failed');
    });
    ...
    it('should handle backend errors in update', async () => {
      ...
      await expect(mockBackend.update('id', { status: 'todo' })).rejects.toThrow('Update failed');
    });
    ...
    it('should handle file operation errors', async () => {
      ...
      await expect(mockBackend.readBeanFile('test.md')).rejects.toThrow('Read failed');
    });
  });
  ...
  describe('multiple operation sequences', () => {
    ...
    it('should handle file operations sequence', async () => {
      ...
      // Edit file
      const edited = await mockBackend.editBeanFile('test.md', 'modified content');
  ...
  describe('concurrent operations', () => {
    it('should handle multiple list calls', async () => {
      ...
      const [list1, list2, list3] = await Promise.all([mockBackend.list(), mockBackend.list(), mockBackend.list()]);
